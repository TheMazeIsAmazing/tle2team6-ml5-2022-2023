...
$productUserId = mysqli_escape_string($db, json_decode(file_get_contents('php://input'), true)['productUserId']); // Get the productUserId from the POST request


if ($productUserId !== null && $productUserId !== '') {
    $query = "DELETE FROM `product_user` WHERE id = '$productUserId'";
    try {
